...
static int __init efi_systab_init(void *phys)
{
	if (efi_enabled(EFI_64BIT)) {
		...
		if (efi_setup) {
			data = early_ioremap(efi_setup, sizeof(*data));
			if (!data)
				return -ENOMEM;
		}
...
static int __init efi_reuse_config(u64 tables, int nr_tables)
{
	...
	p = tablep = early_ioremap(tables, nr_tables * sz);
	if (!p) {
...
static void __init efi_setup_init(void)
{
	...
	sd = early_ioremap(efi_setup, sizeof(struct setup_data));
	if (!sd) {
		pr_warn("early_ioremap setup_data failed\n");
		efi_setup = 0;
		return;
	}
	...
	early_iounmap(sd, sizeof(struct setup_data));
}
...
static int __init map_regions_fixed(void)
{
	...
	s = sizeof(*data) + nr_efi_runtime_map * sizeof(data->map[0]);
	data = early_ioremap(efi_setup, s);
	if (!data)
		return -ENOMEM;
